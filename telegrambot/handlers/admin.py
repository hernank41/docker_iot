...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ["menu_admin", "menu_principal"]:
        ...
        await safe_edit_or_reply(
            query,
            "⚙️ **Menú de Administración Principal:**",
    ...
    # 1. Estado de Máquinas
        ...
        if not estados:
            await safe_edit_or_reply(query, "ℹ️ No hay información de máquinas registrada.", reply_markup=get_admin_menu_keyboard())
            return

        text = "🏭 **ESTADO EN TIEMPO REAL DE MÁQUINAS**\n\n"
        for m in estados:
            estado_str = m.get("estado", "PARADA")
            icon = "🟢" if estado_str == "ACTIVA" else ("🔴" if estado_str == "PARADA_EMERGENCIA" else "🟡")
    ...
    # 2. Herramientas y Desgaste
        ...
        if not herramientas:
            await safe_edit_or_reply(query, "ℹ️ No hay herramientas registradas.", reply_markup=get_admin_menu_keyboard())
            return
        ...
        for h in herramientas:
            ...
            alerta = " ⚠️ **DESGASTE ALTO**" if porcentaje >= 90 else ""
    ...
    # 3.1 Submenú Eliminar Operario
        ...
        operarios = [u for u in usuarios if u["rol"] == "OPERARIO" and u["activo"]]
        if not operarios:
            await safe_edit_or_reply(query, "ℹ️ No hay operarios para eliminar.", reply_markup=get_operarios_menu_keyboard())
            return

        await safe_edit_or_reply(
            query,
            "🗑️ **SELECCIONE EL OPERARIO A ELIMINAR/DESACTIVAR:**",
    ...
    # 3.3 Submenú Stats Operario
        ...
        operarios = [u for u in usuarios if u["rol"] == "OPERARIO" and u["activo"]]
        if not operarios:
            await safe_edit_or_reply(query, "ℹ️ No hay operarios registrados.", reply_markup=get_operarios_menu_keyboard())
            return
    ...
    # 4. Informes & KPIs con Gráficos Matplotlib
        ...
        if not rep:
            await safe_edit_or_reply(query, "ℹ️ No hay registros de actividades en los últimos 7 días.", reply_markup=get_informes_keyboard())
            return
        ...
        if not rep:
            await safe_edit_or_reply(query, "ℹ️ No hay registros de producción en la semana.", reply_markup=get_informes_keyboard())
            return
    ...
    # 5. Menú Configuración Global
        ...
        await safe_edit_or_reply(
            query,
            f"⚙️ **MODIFICAR CONFIGURACIÓN: `{clave}`**\n\n"
