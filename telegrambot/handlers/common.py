...
async def safe_edit_or_reply(query, text: str, reply_markup=None, parse_mode=None):
    """Edita el mensaje actual si es un mensaje de texto. En caso contrario lo recrea limpiamente."""
    try:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("role"):
        ...
        if rol == "ADMIN":
            ...
            msg = (
                f"Sesión de Operario Iniciada: Bienvenido {username}.\n\n"
                f"• Máquina Asignada: {maq_str}\n"
                f"• Herramienta Montada: {herram_str}\n"
                f"• Comentario Activo: {comentario_str}"
            )
            await update.message.reply_text(msg, reply_markup=get_operario_menu_keyboard())
            return

    msg = (
        "Bienvenido al sistema Industrial IoT\n"
        "Por favor, seleccione su Rol de Usuario para iniciar sesión:"
    )
    await update.message.reply_text(msg, reply_markup=get_role_select_keyboard())
...
async def role_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ["show_role_select", "show_start"]:
        ...
        if user_db:
            ...
            context.user_data["username"] = user_db["nombre"]
            ...
            msg = (
                f"Sesión de Operario Iniciada: Bienvenido {user_db['nombre']}.\n\n"
                f"• Máquina Asignada: {maq_str}\n"
                f"• Herramienta Montada: {herram_str}\n"
                f"• Comentario Activo: {comentario_str}"
            )
...
async def login_step_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if estado_login == "admin_username":
        ...
        if res and res.get("status") == "ok":
            ...
            msg = f"Inicio de sesión exitoso. Bienvenido {user_info.get('nombre', username)} (Administrador)."
            await update.message.reply_text(msg, reply_markup=get_admin_menu_keyboard())
        else:
            await update.message.reply_text(
...
async def login_operario_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if len(args) < 1:
        await update.message.reply_text("Uso correcto: /login_operario <nombre_usuario>")
        return
