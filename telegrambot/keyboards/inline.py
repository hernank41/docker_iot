...
def get_operarios_login_keyboard(operarios):
    keyboard = []
    for op in operarios:
        keyboard.append([InlineKeyboardButton(f"👷 {op['nombre']} (@{op['username']})", callback_data=f"login_op_user_{op['username']}")])
...
def get_admin_menu_keyboard():
    keyboard = [
        ...
        [
            InlineKeyboardButton("⚙️ Configuración Sistema", callback_data="admin_config_menu")
        ],
        [
...
def get_operarios_menu_keyboard():
    keyboard = [
        [
            ...
            InlineKeyboardButton("🗑️ Eliminar Operario", callback_data="admin_op_eliminar_menu")
        ],
        [
...
def get_operarios_list_keyboard(operarios, action_prefix: str):
    keyboard = []
    for op in operarios:
        btn_text = f"❌ {op['nombre']} (@{op['username']})" if "del" in action_prefix else f"👤 {op['nombre']} (@{op['username']})"
...
def get_operario_menu_keyboard():
    keyboard = [
        [
            InlineKeyboardButton("⚙️ Asignarme a Máquina", callback_data="op_asignar_maquina"),
